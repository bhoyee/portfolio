...
export const fetchPosts = async () => {
  const res = await fetch(`/api/posts.php`, { headers: { Accept: "application/json" } });
...
export const fetchPostBySlug = async (slug) => {
  const res = await fetch(`/api/posts.php?slug=${encodeURIComponent(slug)}`, { headers: { Accept: "application/json" } });
